...
export default function CheckoutPage() {
  ...
  /*
   * Dar es Salaam delivery zone.
   * Locations outside this box are treated as outside Dar
   * and checkout delivery fee becomes TSh 0.
   */
  ...
  function setLocationForCheckout(
    coordinates: Coordinates,
    locationName: string
  ) {
    ...
    if (!isInsideDarEsSalaam(coordinates)) {
      setDistanceKm(0);
      setDeliveryFee(0);

      setLocationMessage(
        `Location imepatikana • ${locationName} • Nje ya Dar es Salaam — delivery fee TSh 0`
      );
    ...
    setLocationMessage(
      `Location imepatikana • ${locationName} • sasa tafuta umbali wa delivery`
    );
  }
  ...
  async function findLocation(
    ...
    try {
      /*
       * IMPORTANT:
       * Do not append Dar/Tanzania here.
       * The customer can search any place in the world.
       */
  ...
  async function searchLocationAndCalculate() {
    ...
    if (!query) {
      alert(
        "Andika location yako kwanza. Mfano: Sinza Madukani au Nairobi, Kenya"
      );
      return;
    }
    ...
    try {
      ...
      if (!result) {
        ...
        alert(
          `Location "${query}" haijapatikana. Jaribu kuandika jina la eneo, mji, nchi au address kamili.`
        );
      ...
      /*
       * Outside Dar:
       * allow checkout immediately with TSh 0 delivery fee.
       */
      if (!isInsideDarEsSalaam(result.coordinates)) {
        setDistanceKm(0);
        setDeliveryFee(0);

        setLocationMessage(
          `Location imepatikana • ${result.name} • Nje ya Dar es Salaam — delivery fee TSh 0`
        );
      ...
      /*
       * Inside Dar:
       * calculate actual road distance and delivery fee.
       */
      const gamora =
  ...
  function useMyLocation() {
    ...
    setLocationLoading(true);
    setLocationMessage(
      "Inatafuta location yako duniani..."
    );
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          /*
           * Reverse geocode GPS coordinates so the
           * customer gets a real readable address.
           */
          const reverseUrl =
          ...
          if (reverseResponse.ok) {
            ...
            if (
              typeof reverseData?.display_name ===
              "string" &&
              reverseData.display_name.trim()
            ) {
              locationName =
          ...
          /*
           * Outside Dar:
           * no road-distance calculation.
           * Customer can continue checkout with TSh 0 delivery.
           */
          if (!isInsideDarEsSalaam(coordinates)) {
            setDistanceKm(0);
            setDeliveryFee(0);
            ...
            setLocationMessage(
              `Location yako imepatikana • ${locationName} • Nje ya Dar es Salaam — delivery fee TSh 0`
            );
          ...
          /*
           * Inside Dar:
           * calculate actual road distance from Gamora.
           */
          const gamora =
            await getGamoraLocation();
          ...
          if (distance === null) {
            ...
            alert(
              "Location yako imepatikana lakini mfumo umeshindwa kupata umbali wa barabara. Jaribu tena."
            );
          ...
          alert(
            "Location imepatikana lakini imeshindikana kuikamilisha. Jaribu tena."
          );
        } finally {
          setLocationLoading(false);
        }
  ...
  async function placeOrder(
    ...
    if (
      ...
      if (!customerLocation) {
        alert(
          "Tafuta location kwanza ili mfumo uthibitishe eneo lako."
        );
        return;
      }

      if (
        isInsideDarEsSalaam(customerLocation) &&
        distanceKm <= 0
      ) {
        alert(
          "Tafuta location tena ili mfumo upate umbali na delivery fee."
        );
        return;
      }
    }
    ...
    if (!paymentMethod) {
      alert(
        t("Choose payment method.", "Chagua njia ya malipo.")
      );
      return;
    }
    ...
    <main className="min-h-screen bg-slate-50 p-5 pb-10">
      <div className="mx-auto max-w-2xl rounded-3xl bg-white p-5 shadow md:p-4">
        <h1 className="text-lg font-medium text-slate-900">
          {t("GAMORA ONLINE Checkout", "GAMORA ONLINE Malipo")}
        </h1>

        <p className="mt-2 text-xs text-slate-500">
          {t("Enter your details so we can deliver your products.", "Weka taarifa zako ili tukuletee bidhaa.")}
        </p>
<div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
  <h2 className="mb-3 text-xs font-black text-slate-800">
    🛒 {t("Your Products", "Bidhaa zako")}
  </h2>
      ...
      >
        ...
        <div className="mt-6">
          <label className="mb-2 block text-xs font-bold text-slate-700">
            {t("Delivery Method", "Njia ya kupokea bidhaa")}
          </label>
        ...
        >
          ...
          {deliveryMethod ===
            ...
            <>
              <div>
                <label className="mb-2 block text-xs font-bold text-slate-700">
                  {t("Shipping Address", "Anwani ya Usafirishaji")}
                </label>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full rounded-lg border border-slate-200 p-3 text-xs outline-none transition focus:border-sky-700"
                  placeholder={t("Enter your location, e.g. Sinza Madukani", "Andika location yako, mfano: Sinza Madukani")}
                  value={address}
                  onChange={(e) => {
                    setAddress(
                      e.target.value
                ...
                <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
                  ...
                  >
                    {locationLoading
                      ? "⏳ Inatafuta..."
                      : t("🔎 Find Location & Calculate KM", "🔎 Tafuta Location & Hesabu KM")}
                  </button>
                  ...
                  >
                    📍 {t("Use My Location", "Tumia Location Yangu")}
                  </button>
                </div>
                ...
                {searchedLocationName && (
                  <div className="mt-2 rounded-lg bg-slate-50 p-3">
                    <p className="text-xs font-bold text-slate-400">
                      {t("LOCATION FOUND", "LOCATION ILIYOPATIKANA")}
                    </p>
                ...
                <p className="mt-2 text-xs text-slate-400">
                  {t(
                    "Example: Sinza Madukani, Mwenge, Kariakoo, Mbezi Beach.",
                    "Mfano: Sinza Madukani, Mwenge, Kariakoo, Mbezi Beach."
                  )}
                </p>
              </div>
              ...
              <input
                required
                className="w-full rounded-lg border border-slate-200 p-3 text-xs outline-none transition focus:border-sky-700"
                placeholder={t("Street / Area / Ward", "Mtaa / Eneo / Kata")}
                value={area}
                onChange={(e) =>
              ...
              <input
                className="w-full rounded-lg border border-slate-200 p-3 text-xs outline-none transition focus:border-sky-700"
                placeholder={t("Landmark (e.g. near...)", "Landmark (mfano: karibu na...)")}
                value={landmark}
                onChange={(e) =>
                  setLandmark(
                    e.target.value
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-bold text-sky-800">
                      📍 {t("Delivery Estimate", "Makadirio ya Usafirishaji")}
                    </p>

                    <p className="mt-1 text-xs text-slate-500">
                      {t(
                      "The system automatically estimates the road distance.",
                      "Mfumo unakadiria umbali wa barabara automatically."
                    )}
                    </p>
                  </div>
                ...
                {distanceKm > 0 ? (
                  ...
                  <div className="mt-4 rounded-lg bg-white p-4 text-center">
                    <p className="text-xs font-bold text-slate-500">
                      {t("Enter your location above", "Andika location hapo juu")}
                    </p>

                    <p className="mt-1 text-xs text-slate-400">
                      {t("then click", "kisha bonyeza")}
                      <strong>
                        {" "}
                        {t(
                          "Find Location & Calculate KM",
          ...
          >
            {loading
              ? t("Placing Order...", "Inaweka Order...")
              : t("Place Order", "Weka Order")}
          </button>
        </form>
      </div>
    </main>
