...
export default function HomePage() {
  ...
  <div key={product.id} className="min-w-[185px] shrink-0 sm:min-w-[205px] lg:min-w-[215px]">
    <ProductCard product={product} addToCart={addToCart} bestSeller />
  </div>
))}</Carousel>
        </div>
      </section>
      ...
      >
        {image ? (
          <img
            ...
            alt={product.name}
            loading="lazy"
            className="block h-[150px] w-full object-contain transition-transform duration-300 group-hover:scale-[1.025] sm:h-[175px] lg:h-[190px]"
          />
        ) : (
          <div className="flex h-[150px] w-full items-center justify-center bg-white text-4xl text-gray-300 sm:h-[175px] lg:h-[190px]">
            🛍️
          </div>
        )}
